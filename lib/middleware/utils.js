...
lib.tokenAuthentication = function (req, res, next) {
	...
	if (token) {
		...
		console.log("EpiryDate:" + moment(decoded.exp).format('M/D/YYYY') + " ,currentDate:" + moment().format('M/D/YYYY'));
		/*var expiryDiff = moment().diff(decoded.exp, 'minutes 	');
		var expiryDiff = moment(decoded.exp).diff(moment(), 'minutes 	');
...
lib.outputListAssetHistory = function (req, res, next) {
    res.header("content-type", "application/json");
    res.send(200, { "assetHistory" : req.assets});
};
...
lib.outputapiUrl = function (req, res, next) {
    res.header("content-type", "application/json");
    res.send(200, { "HOST_URL" : req.HOST_URL});
};

lib.outputUsers = function (req, res, next) {
    res.header("content-type", "application/json");
    res.send(200, { "users" : req.users});
};
