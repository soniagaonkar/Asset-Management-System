...
assets.acceptRequestPopulateData = function (req, res, next) {
	var requestId = req.param("requestId") ? req.param("requestId") : ""
	requests.findOne({ _id:requestId}, function(err, requestObj) {
		//Set toDate from today's date + requested duration.
		var toDate = moment().add(requestObj.duration, requestObj.durationRange);
		console.log("Request: "+ requestId + ", toDate: " + toDate.format('M/D/YYYY'));
		
	});
}

//Function will set Request status as accepted.
assets.acceptRequest = function (req, res, next) {
	var requestId = req.param("requestId") ? req.param("requestId") : ""
...
assets.assignAsset = function (req, res, next) {
	...
	if(req.authUser && req.authUser.role == "admin")	{
		...
		assetsCollection.findOne({ _id:assetId}, function(err, assetObj) {
			...
			if(assetObj.primaryOwner == req.authUser.loginId)	{ //check logged user is the assetPrimaryOwner
				...
				loginId: req.body.assignTo
					...
					if(user) {
						...
						//update asset obj
						//assetObj.fromDate = fromDate;
						//assetObj.toDate = toDate;
