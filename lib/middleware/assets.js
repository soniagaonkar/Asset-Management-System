...
assets.assignAsset = function (req, res, next) {
	console.log("assignAsset:before");
	assets.validateToken(req, res, function(user,err){
	
	console.log("assignAsset:after");
	console.log("assignAsset:**user.loginId:"+req.loginId);
	if(req.loginId != null)	{
		console.log("assignAsset:Here...");
		res.json( 200, { status: "success", message: 'User token autheticated in assign asset method' });
	}else{
	console.log("assignAsset:ElseHere...");
	res.json( 403, { status: "success", message: 'Else block' });
	}
	});
...
assets.validateToken = function (req, res, next) {
	var token = req.body.token || req.query.token || req.headers['x-access-token'];
	if (token) {
		var decoded = jwt.decode(token, config.SECRET);
		console.log("validateToken:_id:"+decoded.iss);
		User.findOne({
			_id: decoded.iss
			}, function(err, user) {
				if (err) throw err;
				if(user)	{
					...
					console.log("validateToken:req.loginId:"+req.loginId);
					return user;
				}else	{
					return res.status(401).send({ 
					success: false, 
