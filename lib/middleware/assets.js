...
assets.requestAsset = function (req, res, next) {
	...
	if(!validator.isInt(req.body.duration)){
		res.json( 400, { status: "failure", message: 'Invalid duration.' });
	}
	else if(req.body.durationRange == "weeks" || req.body.durationRange == "months" || req.body.durationRange == "days")	{
		assetsCollection.findOne({ _id:assetId}, function(err, assetObj) {
			if (err) 
				res.json(400,{ status: "failure", message: err });
			else	{				
...
/**
The viewRequests method user can view the requests.
@If admin user is logged in > user can view assigned requests.
@If user is logged in > user can view created requests.
*/
assets.viewRequests = function (req, res, next) {
        ...
        requests.find({ }, function(err, requestData) {
            if (err) 
				...
				console.log("list:"+requestData.length);
				if(req.authUser && req.authUser.role == "admin"){
					requestList = _.where(requestData, {primaryOwnerId: req.authUser.loginId});
				}else	{
					 requestList = _.where(requestData, {requestorId: req.authUser.loginId});
				}
                async.waterfall([
                    function(callback) {
                        var assetsArr = []
